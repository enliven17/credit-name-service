...
interface ICreditNameService {
    function ownerOf(string calldata name) external view returns (address);
    function marketTransfer(string calldata name, address from, address to) external;
}

/// @title Marketplace for Credit Name Service
contract CreditNameMarketplace is Ownable {
    struct Listing {
        address seller;
        uint256 price;
        bool active;
    ...
    uint256 public constant LISTING_FEE = 100 ether; // 100 tCTC
    ICreditNameService public immutable nameService;
    ...
    constructor(address ns, address initialOwner) {
        ...
        nameService = ICreditNameService(ns);
