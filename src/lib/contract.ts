...
// Credit Name Service ABI
const CREDIT_NAME_SERVICE_ABI = [
...
export class CreditNameServiceContract {
  ...
  constructor(provider: unknown) {
    ...
    this.contract = new ethers.Contract(CONTRACT_ADDRESS, CREDIT_NAME_SERVICE_ABI, provider)
  ...
  async registerDomain(domainName: string): Promise<string> {
    try {
      ...
      console.log('💰 Payment amount:', ethers.formatEther(DOMAIN_PRICE), 'tCTC');
      ...
      console.log('💰 Using fixed domain price: 1000 tCTC');
      ...
      if (error.message?.includes('insufficient funds')) {
        throw new Error('Insufficient funds to register domain (need 1000 tCTC + gas)');
      }
...
export const getCreditContract = (provider: unknown): CreditNameServiceContract => {
  try {
    // For modern providers (MetaMask, etc.)
    if (provider.request) {
      const ethersProvider = new ethers.BrowserProvider(provider)
      return new CreditNameServiceContract(ethersProvider)
    }
    // Fallback for other providers
    return new CreditNameServiceContract(provider)
  } catch (error) {
