...
export default function CreateListingModal({ domain, sellerAddress, onClose, onListed }: Props) {
  ...
  const submit = async () => {
    ...
    if (!price || isNaN(Number(price)) || Number(price) <= 0) {
      setError('Enter a valid price in CTC');
      return;
    }
    if (!window.ethereum) {
    ...
    try {
      setLoading(true);
      const name = domain.name.replace('.ctc', '');
      ...
      const nsContract = new ethers.Contract(
        process.env.NEXT_PUBLIC_CREDIT_CONTRACT_ADDRESS || '',
  ...
  const marketplaceCreate = async (domainId: string, seller: string, priceCtc: string, tx?: string) => {
    await domainService; // keep import used
    console.log('📝 Creating marketplace listing in Supabase...');
    console.log('Domain ID:', domainId);
    console.log('Seller:', seller);
    console.log('Price:', priceCtc);
    ...
    try {
      const result = await marketplaceService.createListing(domainId, seller, priceCtc, 'fixed_price', undefined, undefined, tx);
  ...
  return (
    <Backdrop onClick={onClose}>
      <Card onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ color: 'rgba(255,255,255,0.7)', marginBottom: 8 }}>Enter price in CTC</div>
